...
import { Star, CheckCircle2, Circle, MessageSquare, ClipboardList, RefreshCw } from "lucide-react";
import { useLocation } from "wouter";

type DailyItem = {
  id: string;
  type: "needs_offer" | "task";
  label: string;
  detail?: string;
  link?: string;
  done: boolean;
...
export default function DailyZero() {
  ...
  const { data: rawData, isLoading, refetch } = trpc.tasks.dailyZero.useQuery();

  // Normalize the { pendingTasks, needsOfferConvs } response into a flat DailyItem array
  const items: DailyItem[] = useMemo(() => {
    if (!rawData) return [];
    const result: DailyItem[] = [];

    // Needs Offer conversations
    const needsOffer = (rawData as any).needsOfferConvs ?? [];
    for (const conv of needsOffer) {
      result.push({
        id: `conv-${conv.id}`,
        type: "needs_offer",
        label: conv.contactName || conv.contactPhone || "Unknown Contact",
        detail: conv.contactPhone,
        link: `/messenger/${conv.id}`,
        done: false,
      });
    }

    // Pending tasks
    const pendingTasks = (rawData as any).pendingTasks ?? [];

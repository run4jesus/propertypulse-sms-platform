  ...
  contactLists: router({
    ...
    preImportCheck: protectedProcedure
      .input(z.object({
        ...
        excludeLitigators: z.boolean().default(true),
        excludeExisting: z.boolean().default(false),
      }))
      .query(async ({ ctx, input }) => {
        const db = await getDb();
        if (!db) return { totalCount: 0, dncCount: 0, litigatorCount: 0, duplicateCount: 0, cleanCount: 0 };
        ...
        for (const phone of input.phones) {
          ...
          if (seenInBatch.has(normalized)) {
            duplicateCount++;
          } else if (input.excludeDnc && dncPhones.has(normalized)) {
            dncCount++;
          } else if (input.excludeLitigators && litigatorPhones.has(normalized)) {
            litigatorCount++;
          } else if (input.excludeExisting && existingPhones.has(normalized)) {
            duplicateCount++;
          }
          seenInBatch.add(normalized);
        }
        const uniqueInBatch = seenInBatch.size;
        const cleanCount = uniqueInBatch - dncCount - litigatorCount - duplicateCount;
        return {
          totalCount: uniqueInBatch,
          dncCount,
          litigatorCount,
          duplicateCount,
  ...
  campaigns: router({
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        sendMode: z.enum(["automated", "manual"]).default("automated"),
    ...
    update: protectedProcedure
      .input(z.object({
        ...
        sendMode: z.enum(["automated", "manual"]).optional(),
